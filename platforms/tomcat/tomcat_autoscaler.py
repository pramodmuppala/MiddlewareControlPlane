#!/usr/bin/env python3
"""
Tomcat metrics-based autoscaler with optional LLM policy support.
"""
...
class Config:
    ...
    catalina_base_root: str = "/opt/tomcat"
    ...
    state_file: str = "/tmp/tomcat_autoscaler_state.json"
    dry_run: bool = False
    use_llm: bool = False
...
def instance_http_port(cfg: Config, name: str) -> int:
    num = instance_number(name, cfg.instance_name_prefix)
    offset = num - cfg.instance_start_index
...
def probe_instances(cfg: Config, instances: List[str]) -> List[ProbeResult]:
    health_path = cfg.health_path if cfg.health_path.startswith("/") else f"/{cfg.health_path}"
    results = []
...
def average_latency_ms(results: List[ProbeResult]) -> Optional[float]:
    samples = [r.latency_ms for r in results if r.healthy and r.latency_ms is not None]
...
def build_ansible_cmd(cfg: Config, target_count: int, destroy_enabled: bool) -> List[str]:
    extra_vars = {"tomcat": {"instance_count": target_count, "destroy_enabled": destroy_enabled}}
...
def decide_target_count_rules(cfg: Config, state: State, current_count: int, cpu_percent: float, mem_percent: float, avg_latency: Optional[float]) -> Tuple[int, str]:
...
def decide_target_count_llm(cfg: Config, state: State, current_count: int, healthy_count: int, cpu_percent: float, mem_percent: float, avg_latency: Optional[float], instances: List[str]) -> Tuple[int, str]:
...
def run_once(cfg: Config) -> int:
    ...
    instances = discover_instances(cfg.catalina_base_root, cfg.instance_name_prefix)
    ...
    probe_summary = ", ".join(f"{p.instance_name}:{'up' if p.healthy else 'down'}@{p.port}" for p in probes) or "no instances discovered"
    log(f"instances={current_count} healthy={healthy_count} cpu={cpu_percent:.2f}% mem={mem_percent:.2f}% avg_latency_ms={avg_latency if avg_latency is not None else 'n/a'} probes=[{probe_summary}]")
    if cfg.use_llm:
        target_count, reason = decide_target_count_llm(cfg, state, current_count, healthy_count, cpu_percent, mem_percent, avg_latency, instances)
    else:
        target_count, reason = decide_target_count_rules(cfg, state, current_count, cpu_percent, mem_percent, avg_latency)

    log(f"decision current={current_count} target={target_count} up_streak={state.up_streak} down_streak={state.down_streak} reason={reason}")
...
def parse_args() -> Tuple[Config, bool]:
    parser = argparse.ArgumentParser(description="Tomcat metrics-based autoscaler")
    ...
    parser.add_argument("--catalina-base-root", default="/opt/tomcat")
    ...
    parser.add_argument("--state-file", default="/tmp/tomcat_autoscaler_state.json")
    ...
    cfg = Config(
        ...
        catalina_base_root=args.catalina_base_root,
